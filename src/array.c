...
/* CONVERTS X from two-bit representation to integer and writes string to OUT.
...
x_count (array_t a, array_t mask)
{
  ...
  return __builtin_popcountll (tmp); // C, not C++, counts set bits ('1')
}
/*
	CREATES array_t[] with length = len * 2 / sizeof(array_t)	 
*/

array_t *
...

void // IS IT UNION? I THINK NO. WHAT IS IT?
...
/* 
	CREATES array of (array_t *) - all the wildcards that form the complementation of a
	a - 10 10 01 11 01 10	-	1 1 0 x 0 1
	{
		11 11 11 11 11 01	-	x x x x x 0
		11 11 11 11 10 11	-	x x x x 1 x
		11 11 10 11 11 11	-	x x 1 x x x
		11 01 11 11 11 11	-	x 0 x x x x
		01 11 11 11 11 11	-	0 x x x x x
	}
	RETURNS if there are any wildcards that form complementation
*/
bool
...
/*
	CREATES res - array of wildcards - intersection of a and wildcards that complement b
	RETURNS false if there are no complementation to b
			or amount of wildcards in res
*/
bool
...
/*
	FIILS res with a wildcard - a & b
	RETURNS if intersection of a and b is not void (if z appears)
*/
bool
...
/*
	RETURNS res if no z, NULL if a & b has z
*/
